...
- [ ] Measure model pass-rate per language like we did for English (the yield test is English-only today; review 2026-07-23 saw a German request served curated, suggesting the German model path may reject often; extend `generatesAnAcceptableStoryWithinAFewAttempts` across StoryLanguage)
