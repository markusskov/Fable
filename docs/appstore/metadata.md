...
- **Raw app captures: `docs/appstore/screenshots/<language>/01..09-*.png`** —
  every scene of the app in all seven shipped languages (en, nb, de, es, fr,
  it, pt-BR), regenerated by `scripts/capture-screenshots.sh`. These are the
  honest UI, not marketing frames: use them to review translations and to see
  what a family in each language actually gets. The lane finds every control
  by accessibility identifier, so it does not break when copy changes, and it
  leaves the comfort-object field empty so each language's own default is
  woven into the story rather than an English word the tester typed.
