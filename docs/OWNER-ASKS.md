...
0. **REJECTED 2026-07-28 (automated, metadata-only, guideline 3.1.2) — fix is ready, ~10 minutes of ASC work.** Exactly the predicted subscription-metadata class: the App Description lacks a functional Terms of Use (EULA) link. The subscriptions themselves were returned only as "associated items"; nothing about them needs editing. All seven metadata packs now carry the legal links (Terms → Apple's standard EULA, plus Privacy) just before the goodnight line, and the limits check passes. Your steps, in order:
   1. ASC → Fable → the rejected 1.0 submission → App Description for each LIVE localization (en, de, nb): re-paste the description from `docs/appstore/metadata.md` / `metadata-de.md` / `metadata-nb.md` (only the two link lines before the goodnight line are new; pasting the whole description is safest).
   2. Do NOT edit the subscription items or the build — they resubmit as-is.
   3. Resubmit the same submission (build 5). Automated metadata rejections typically re-review fast.
   4. If anything else comes back, open a session with the message; the rejection playbook is docs/launch/first-day-checklist.md.
...
2. **SUBMISSION DAY — the App Store Connect work, in order.** Build 3 was withdrawn (it predated the safety fixes and had no in-app legal links). Everything on the code side for a replacement is done: build number is now **4**, the privacy manifest ships, and the listing text below is corrected. What is left is yours.
   ...
   **a2. German prices differ from the base tier (owner found this 2026-07-27):** 5,99 EUR/month and 44,99 EUR/year, not the 4,99/39,99 the localized packs assumed. Every localized description hardcoded prices, and all of them are now stripped: metadata localization follows LANGUAGE, not storefront, so a German description reaches German speakers in Switzerland (CHF) and a Spanish one reaches all of Latin America. The packs now point at the in-app paywall, which shows StoreKit's real localized price. Nothing to maintain when Apple adjusts a tier. **Please re-paste the German description**, since the live one still carries the old figures.
...
3. **iPad screenshots: CAPTURED and staged — upload with the v1.1 listing.** All seven languages, nine scenes each, 2064×2752, in `docs/appstore/screenshots/ipad-13/<lang>/` (raw, unframed — ASC accepts these directly; frame in Figma only if you want caption parity with the iPhone set). Captured 2026-07-28 via `DEVICE="iPad Pro 13-inch (M5)" scripts/capture-screenshots.sh`.
