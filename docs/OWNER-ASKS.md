...
3. **iPad screenshots will be required for the v1.1 submission — no action yet.** The iPad reading-column layout is merged, so the next binary uploaded to ASC declares iPad support and the listing then requires a 13" iPad screenshot set (2064×2752). The `IPadLayoutTests` lane in the FableScreenshots scheme captures the scenes; a session will stage and export the set alongside the v1.1 submission prep, and you upload it with the rest of the listing.
